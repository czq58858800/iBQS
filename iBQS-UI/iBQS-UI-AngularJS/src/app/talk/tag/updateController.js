...
angular.module('app')
    .controller('tagUpdateController', ['$scope', '$rootScope', '$state', '$timeout', 'toaster',
        function($scope, $rootScope, $state, $timeout, toaster) {
            var title = "";
            $scope.myImage='';
            $scope.record = {};
            if($state.includes('**.tag.update')){
                title="编辑广告";
                var id = $state.params.id;
                activate(id);
                validate(id);
            }else if($state.includes('**.tag.create')){
                title="添加广告";
                validate(null);
            }
            ...
            $scope.$on('$viewContentLoaded', function() {
                var small = document.getElementById("tag_image");
            ...
            function readImageFile(){
                ...
                reader.onload = function(e){
                    ...
                    small_tag_image.innerHTML = '<div class="sitetip"></div><img width="375" height="94" src="'+this.result+'" alt= ""/>';
                    $("#small_tag_image img").each(function() {
