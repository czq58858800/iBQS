...
public final class ThirdPartyLoginHelper {
	...
	public static final ThirdPartyUser getQQUserinfo(String loginDevice,String token, String openid) throws Exception {
		...
		if (json.getIntValue("ret") == 0) {
			...
			user.setThirdpartyName(json.getString("nickname"));
	...
	public static final ThirdPartyUser getSinaUserinfo(String token, String uid) throws Exception {
		...
		user.setThirdpartyName(nickName);
		if ("f".equals(json.getString("gender"))) {
			user.setGender("0");
		} else if ("m".equals(json.getString("gender"))){
			user.setGender("1");
		} else {
			user.setGender("2");
		}
		if (json.containsKey("verified") && json.getBoolean("verified")) {
			user.setVerified(json.getBoolean("verified"));
			user.setVerifiedReason(json.getString("verified_reason"));
		}
