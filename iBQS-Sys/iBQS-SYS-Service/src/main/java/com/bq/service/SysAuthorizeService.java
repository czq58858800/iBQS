...
import com.bq.mapper.SysAuthorizeMapper;
import com.bq.mapper.SysRoleMenuMapper;
import com.bq.mapper.SysUserMenuMapper;
import com.bq.mapper.SysUserRoleMapper;
import com.bq.model.SysDic;
import com.bq.model.SysRoleMenu;
import com.bq.model.SysMenu;
import com.bq.model.SysUserRole;
...
public class SysAuthorizeService {
    ...
    public List<String> queryMenuIdsByUserId(String userId) {
        List<String> resultList = InstanceUtil.newArrayList();
        List<String> list = sysUserMenuMapper.queryMenuIdsByUserId(userId);
        for (String id : list) {
    ...
    public List<String> queryMenuIdsByRoleId(String roleId) {
        List<String> resultList = InstanceUtil.newArrayList();
        List<String> list = sysRoleMenuMapper.queryMenuIdsByRoleId(roleId);
        for (String id : list) {
            resultList.add(id.toString());
        }
        return resultList;
    ...
    public List<SysMenu> queryAuthorizeByUserId(String userId) {
        ...
        for (SysMenu sysMenu : menus) {
            if (StringUtils.isBlank(sysMenu.getParentId()) || StringUtils.equals(sysMenu.getParentId(),"0")) {
                sysMenu.setLeaf(0);
    ...
    // 递归获取子菜单
    private List<SysMenu> getChildMenu(Map<String, List<SysMenu>> map, String id) {
